...
import { Copy, Share2, Shield, Activity, Zap, CheckCircle2, ExternalLink, Clock, TrendingUp, Wallet } from "lucide-react";
              ...
              {[
                ...
                <div key={item.label}>
                  ...
                  <div className="flex items-center gap-1.5">
                    <p className={`text-sm ${item.mono ? "font-mono" : ""} ${item.color === "green" ? "text-green font-semibold" : "text-foreground"}`}>
                      {item.value}
                    </p>
                    {item.copy && (
        ...
        <div className="grid lg:grid-cols-3 gap-0">
          ...
          <div className="lg:col-span-2 lg:pr-10 lg:border-r border-border py-10">
            <div className="flex justify-between items-baseline border-b border-border pb-3 mb-0">
              ...
              <span className="font-mono text-[11px] text-muted-foreground/40">{agent.activity.length} events</span>
            </div>
            <div>
                ...
                return (
                  ...
                  >
                    <Icon className={`w-3.5 h-3.5 mt-0.5 ${
                      act.status === "success" ? "text-green/60" :
                      act.status === "failed" ? "text-destructive/60" : "text-amber/60"
                    }`} />
                      ...
                      <div className="flex flex-wrap items-center gap-3">
                        ...
                        {act.txHash && (
                          <a href={`https://explorer.solana.com/tx/${act.txHash}?cluster=devnet`}
                            target="_blank" rel="noopener noreferrer"
                            className="font-mono text-xs text-green link-underline">
