...
function JournalRow({ agent }: { agent: Agent }) {
  ...
  return (
    <Link to={`/agent/${agent.id}`} className="group block" onMouseEnter={() => { if (previewRef.current) scramble(previewRef.current, previewText); }}>
      <div
...
export default function Agents() {
  ...
  const filtered = useMemo(() => {
    ...
    // Framework filter
    if (framework !== "All") {
      list = list.filter((a) => a.framework === framework);
    }
